...
describe('Application', function () {
    ...
    var org = new config_1.Domain({
        ...
        domains: [],
        resources: {
            containerRepository: new config_1.ContainerRepositoryResource({
                env: 'e',
                name: 'n',
            }),
        }
    ...
    describe('validation', function () {
        it('throws exception when no containerRepository is configured', function () {
            expect(function () {
                var orgWithNoCR = new config_1.Domain({
                    env: 'e',
                    name: 'n',
                    location: 'l',
                    gitRepositoryUrl: 'g',
    ...
    describe('containerAppName', function () {
        var resourceGroups = new config_1.ResourceGroups({
            env: 'e',
            name: 'n'
        });
        it('with env', function () {
            var domain = new config_1.Domain({
                env: 'e',
                name: 'n',
                location: 'l',
                gitRepositoryUrl: 'g',
                ...
                domains: [],
                resources: {
                    containerRepository: new config_1.ContainerRepositoryResource({ env: 'e', name: 'n' }),
                }
            });
            var appWithEnv = new config_1.Application({
                name: 'n'
            }, domain);
            expect(appWithEnv.containerAppName).toEqual('n-e');
        });
        it('with env and instance', function () {
            var domainWithInst = new config_1.Domain({
                env: 'e',
                instance: 'i',
                name: 'n',
                location: 'l',
            ...
            var appWithEnvAndInst = new config_1.Application({
                name: 'n'
            }, domainWithInst);
        ...
        it('name cannot be greater than 32 characters', function () {
            ...
            var appWithLongName = new config_1.Application({
...
describe('Domain', function () {
    ...
    describe('validation', function () {
        ...
        it('valid Domain throws no exceptions', function () {
            var org = new config_1.Domain(__assign(__assign({}, opts), { location: 'l', gitRepositoryUrl: 'g', applications: [] }));
        });
        it('env length must be 4 or less', function () {
            expect(function () {
                new config_1.Domain(__assign(__assign({}, opts), { env: '12345', location: 'l', gitRepositoryUrl: 'g', applications: [] }));
            }).toThrow('Configuration Error - Env cannot be longer than 4 characters');
        });
        it('4 character env length is ok', function () {
            new config_1.Domain(__assign(__assign({}, opts), { env: '1234', location: 'l', gitRepositoryUrl: 'g', applications: [] }));
        });
    ...
    describe('valid Domain instance', function () {
        ...
        var org = new config_1.Domain(__assign(__assign({}, orgOpts), { location: 'CentralUs', gitRepositoryUrl: 'g', bootstrapUsers: [
                'foo@bar.com',
                'bizz@buzz.com',
            ], resources: {
                b2c: new config_1.B2CResource(orgOpts),
                containerRepository: new config_1.ContainerRepositoryResource(orgOpts),
                keyVault: new config_1.KeyVaultResource(orgOpts),
            }, applications: [] }));
        var domain = new config_1.Domain({
            env: env,
            instance: instance,
    ...
    describe('Instantiating Domain', function () {
        ...
        var org = new config_1.Domain(__assign(__assign({}, domainOpts), { location: 'CentralUs', env: 'dev', instance: 'ci', gitRepositoryUrl: 'http://', resources: {
                b2c: new config_1.B2CResource(domainOpts),
                containerRepository: new config_1.ContainerRepositoryResource(domainOpts),
                keyVault: new config_1.KeyVaultResource(domainOpts),
            }, applications: [
                {
                    name: 'enrich-srv',
                    enrichApi: true,
        ...
        it('gets domain by name from domain', function () {
            expect(org.getDomainByName('MajorLeagueMiruken').name).toEqual('majorleaguemiruken');
        });
        it('gets domain by name from child domains', function () {
            expect(org.getDomainByName('billing').name).toEqual('billing');
        });
        it('throw exception when domain is not found', function () {
            org.applications[0].enrichApi = false;
            expect(function () { org.getDomainByName('I dont exist'); }).toThrow('Domain with name I dont exist not found');
        });
...
describe('ContainerRepositoryResource', function () {
    it('exitsts', function () {
        expect(config_1.ContainerRepositoryResource).toBeDefined();
    });
    describe('validation', function () {
        it('valid ContainerRepositoryResource throws no exceptions', function () {
            new config_1.ContainerRepositoryResource({
                env: 'e',
                name: 'n',
            });
        });
    ...
    describe('valid ContainerRepositoryResource instance', function () {
        var cr = new config_1.ContainerRepositoryResource({
            env: 'e',
            name: 'My-Name'
        });
...
describe('KeyVault', function () {
    it('exists', function () {
        expect(config_1.KeyVaultResource).toBeDefined();
    });
    describe('validation', function () {
        it('name cannot be more than 24 characters', function () {
            expect(function () {
                var keyVault = new config_1.KeyVaultResource({
...
describe('B2CResource', function () {
    it('exitsts', function () {
        expect(config_1.B2CResource).toBeDefined();
    });
    describe('valid b2c instance', function () {
        var b2c = new config_1.B2CResource({
            name: 'Major-League-Miruken',
            env: 'dev'
        });
